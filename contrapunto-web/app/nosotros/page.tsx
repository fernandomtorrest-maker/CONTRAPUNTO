...
// Generate list of 12 pages (slides)
const totalPages = 12;
const presentationPages = Array.from({ length: totalPages }, (_, i) => ({
  pageNumber: i + 1,
  src: `/images/nosotros/page_${i + 1}.webp`
}));
...
export default function NosotrosPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#0f0e0c] text-cream overflow-hidden selection:bg-sand selection:text-carbon font-body font-sans">
      {/* Navbar */}
      <Navbar />
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div className="space-y-4">
              ...
              <p className="text-sm text-cream/70 leading-relaxed max-w-lg">
                Conoce a fondo nuestra trayectoria, propuesta de valor y el equipo multidisciplinario 
                detrás de Constructora Contrapunto.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
              ...
              {/* Descargar PDF */}
              <a
                href="/images/Presentacion Corporativa.pdf"
                download="Presentacion Corporativa.pdf"
              ...
              >
                <Download size={14} /> Descargar PDF (46 MB)
              </a>
            </div>
          </div>
        </div>
      ...
      <section className="py-12 bg-[#0c0c0a] min-h-[60vh] flex items-center justify-center">
        <div className="container-base max-w-6xl px-6 md:px-8 w-full">
          ...
          {/* MODO LECTOR (Especialmente diseñado para slides 16:9 horizontales) */}
          {viewMode === 'reader' && (
            <div className="flex flex-col items-center gap-8">
              
              {/* Contenedor del Slide */}
              <div className="relative w-full max-w-4xl aspect-[16/9] bg-[#12110e] border border-white/10 rounded-xl overflow-hidden shadow-2xl shadow-black/80 flex items-center justify-center group select-none">
                
                {/* Slide en visualización */}
                <div 
                ...
                >
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <Image
                        src={presentationPages[currentPageIdx].src}
                        alt={`Presentación - Diapositiva ${currentPageIdx + 1}`}
                        fill
                        priority
                ...
                <button
                  onClick={prevPage}
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 hover:bg-black/90 border border-white/10 text-white flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 cursor-pointer shadow-lg"
                ...
                <button
                  onClick={nextPage}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 hover:bg-black/90 border border-white/10 text-white flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 cursor-pointer shadow-lg"
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-4xl justify-between border-t border-white/5 pt-4">
                {/* Indicador de Página */}
                <div className="text-xs uppercase tracking-widest font-bold text-cream/60">
                  Diapositiva <span className="text-sand font-extrabold">{currentPageIdx + 1}</span> de {totalPages}
                </div>
              ...
              <div className="w-full max-w-4xl flex gap-3 overflow-x-auto py-2.5 scrollbar-thin scrollbar-thumb-white/10 justify-start sm:justify-center">
                {presentationPages.map((page, idx) => (
                  <button
                    key={page.pageNumber}
                    onClick={() => setCurrentPageIdx(idx)}
                    className={`relative w-20 aspect-[16/9] rounded border overflow-hidden transition-all shrink-0 cursor-pointer ${
                      currentPageIdx === idx 
          ...
          {viewMode === 'grid' && (
            ...
            >
              {presentationPages.map((page, idx) => (
                ...
                >
                  {/* Imagen Miniatura */}
                  <div className="relative aspect-[16/9] w-full overflow-hidden bg-stone-dark">
                    <Image
                      src={page.src}
                      alt={`Presentación - Diapositiva ${page.pageNumber}`}
                      fill
          ...
          >
            ...
            <div className="flex justify-between items-center w-full max-w-6xl mx-auto py-2 z-10">
              <span className="text-[10px] font-extrabold text-sand uppercase tracking-[0.2em]">
                Presentación Corporativa — Ampliado
              </span>
            ...
            {/* Imagen Ampliada */}
            <div className="relative w-full max-w-5xl mx-auto aspect-[16/9] max-h-[82vh] flex items-center justify-center my-auto">
              <div 
                className="relative w-full h-full"
              ...
              >
                <Image
                  src={presentationPages[currentPageIdx].src}
                  alt={`Diapositiva Ampliada ${currentPageIdx + 1}`}
                  fill
                  priority
                  className="object-contain"
                />
